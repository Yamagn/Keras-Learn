...
def model_disciriminator():
    nch = 256
    h = 5
    reg = lambda: L1L2(l1=1e-7, l2=1e-7)

    c1 = Conv2D(int(nch / 4), (h, h), padding="same", kernel_regularizer=reg(), input_shape=dim_ordering_shape((3, 32, 32)))
...
def cifar10_data():
    ...
    return cifar10_process(x_train), cifar10_process(x_test)
...
def main():
    ...
    # discriminator (x -> y)
    discriminator = model_disciriminator()
    ...
    example_gan(AdversarialOptimizerSimultaneous(),
                ...
                nb_epoch=500, generator=generator,
